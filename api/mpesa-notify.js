...
export const handler = async (event) => {
  ...
  try {
    ...
    // Fallback SMS parser: If the Android app sends the raw SMS string, extract details automatically
    const fullText = sms || body.body || body.message || '';
    if (fullText && (!transaction_code || !amount || !sender)) {
      const codeMatch = fullText.match(/^[A-Z0-9]{10}/);
      if (codeMatch && !transaction_code) transaction_code = codeMatch[0];

      const amtMatch = fullText.match(/Ksh\s*([\d,]+\.\d{2})/i) || fullText.match(/Ksh\s*([\d,]+)/i);
      if (amtMatch && !amount) amount = parseFloat(amtMatch[1].replace(/,/g, ''));

      // Extract sender: "from JOHN DOE 2547..." or "from JOHN DOE on..."
      let fromMatch = fullText.match(/from\s+(.*?)\s+(?:254\d|07\d|01\d)/i);
      if (!fromMatch) fromMatch = fullText.match(/from\s+([A-Z\s]+?)\s+on/i);
      if (fromMatch && !sender) sender = fromMatch[1].trim();

      const phoneMatch = fullText.match(/(?:254\d{9}|07\d{8}|01\d{8})/);
    ...
    const dbToken = (bodyDbToken || process.env.TURSO_AUTH_TOKEN || '').replace(/\s+/g, '');
