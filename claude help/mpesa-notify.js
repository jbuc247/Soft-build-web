...
export const handler = async (event) => {
  ...
  try {
    ...
    const dbUrl = (bodyUrl || process.env.TURSO_DATABASE_URL || '').trim().replace(/["'\s]/g, '').replace(/^libsql:\/\//, 'https://');
    const dbToken = (bodyUrl ? (bodyDbToken || '') : (bodyDbToken || process.env.TURSO_AUTH_TOKEN || '')).trim().replace(/["'\s]/g, '');
    console.log(`[DEBUG] DB URL: ${dbUrl}`);
    console.log(`[DEBUG] DB Token Length: ${dbToken.length}, Ends with: ${dbToken.slice(-5)}`);
