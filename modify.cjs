...
// 1. Replace validPins
content = content.replace(/private val validPins = listOf\("[^"]+", "[^"]+"\)/, 'private val LICENSE_API_URL = "https://softlybuilt-license-server.netlify.app/.netlify/functions/verify-license"');
...
content = content.replace(/private val inactivityHandler = Handler\(Looper\.getMainLooper\(\)\)\s*private val lockRunnable = Runnable \{ lockApp\(\) \}/, '');
...
const btn_logic = `btnUnlock?.setOnClickListener {
...
content = content.replace(/btnUnlock\?\.setOnClickListener \{[^}]+\}[^}]+\}/, btn_logic);
...
content = content.replace(/override fun onUserInteraction\(\) \{[\s\S]+?\}\s*private fun resetInactivityTimer\(\) \{[\s\S]+?\}/, '');
